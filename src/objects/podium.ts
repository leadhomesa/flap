...
class Podium extends Phaser.GameObjects.Rectangle {
  ...
  constructor({ scene, y }: PodiumOptions) {
    ...
    const height = Math.floor(window.innerWidth * 0.27) - 28;
  ...
  private createImage(): void {
    const width = window.innerWidth;
    const height = Math.floor(window.innerWidth * 0.27);

    if (this._currentScene) {
        const podium = this._currentScene.add.image(0, this.y, "podium");
